...
- **Releases** attach the CI-built binary (reproducible) rather than a local build: download the artifact from the `main` run → **sign it locally if a certificate is available** (Certum Open Source cloud cert via SimplySign Desktop; skipped so far, see below) → `gh release create vX.Y.Z --target main <exe>` (this creates the tag remotely). Bump `<Version>`/`<FileVersion>` in `MppViewer.csproj` for each release — **and the two version mentions in README**: the `> **Early version (X.Y.Z)**` banner near the top and the `Pin a specific version (e.g. vX.Y.Z)` example in the organization section. The status bar and About dialog read the assembly, so they follow the csproj on their own; README does not. Verify with `grep -n '1\.[0-9]\.[0-9]' README.md` — it should only show the version just released. (1.3.0 shipped with the banner still reading 1.2.2; caught afterwards.) The full signed-release procedure (purchase, one-time setup, `signtool` command, verification) is in [`docs/RELEASE.md`](docs/RELEASE.md).
